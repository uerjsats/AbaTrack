...
class MainWindow(QMainWindow):
    ...
    def iniciarLeitura(self):
        if (self.threadSerial == None and self.threadRep == None) or not self.threadSerial.isRunning() or not self.threadRep.isRunning():
            self.threadRep = ThreadLeituraRepositorio(self.repositorio)
            self.threadSerial = ThreadLeituraSerial(self.adaptador)

            self.threadRep.dadoLido.connect(self.graficoDinamico.atualizarGrafico)
    ...
    def pressionarDesconectar(self):
        ...
        if self.threadRep != None and self.threadSerial != None:
            self.threadRep.stop()
            self.threadSerial.stop()
