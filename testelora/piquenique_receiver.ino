...
void OnRxDone(uint8_t *payload, uint16_t size, int16_t rssi, int8_t snr) {
    ...
    if (payload[0] == 42) {  
        // Converte os dados para uma string corretamente
        String mensagem = "";
        for (int i = 1; i < size; i++) {  // Ignora o primeiro byte (endereço)
            mensagem += (char)payload[i];
        }

        Serial.println(mensagem);  // Agora a string deve ser UTF-8 válida
        ...
        factory_display.drawString(0, 30, mensagem); // Agora a string está correta
