...
class AdaptadorArduino:
    ...
    def conectar(self):
        try:
            ...
            print(f"[DEBUG] Conexão estabelecida na porta {self.configs.portaArduino} com baud rate {self.configs.baudRate}")
        except serial.SerialException as e:
            print(f"[ERRO] Falha ao conectar na porta {self.configs.portaArduino}: {e}")
            self.conexao = None
